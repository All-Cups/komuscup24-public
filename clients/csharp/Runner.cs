...
namespace Komus24
{
    public class Runner
    {
        ...
        public void Run()
        {
            ...
            while (running)
            {
                switch (Komus24.Codegame.ServerMessage.ReadFrom(reader))
                {
                    case Komus24.Codegame.ServerMessage.UpdateConstants message:
                        myStrategy = new MyStrategy(message.Constants);
                        break;
                    case Komus24.Codegame.ServerMessage.GetOrder message:
                        new Komus24.Codegame.ClientMessage.OrderMessage(myStrategy.GetOrder(message.PlayerView, message.DebugAvailable ? debugInterface : null)).WriteTo(writer);
                        writer.Flush();
                        break;
                    case Komus24.Codegame.ServerMessage.Finish message:
                        running = false;
                        myStrategy.Finish();
                        break;
                    case Komus24.Codegame.ServerMessage.DebugUpdate message:
                        myStrategy.DebugUpdate(message.DisplayedTick, debugInterface);
                        new Komus24.Codegame.ClientMessage.DebugUpdateDone().WriteTo(writer);
                        writer.Flush();
                        break;
                    default:
